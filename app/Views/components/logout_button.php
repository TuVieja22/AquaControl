<?php
$logoutAction = $action ?? base_url('auth/logout');
$logoutLabel = $label ?? 'Salir';
$logoutAriaLabel = $ariaLabel ?? 'Cerrar sesion';
$logoutClass = trim('Btn ' . ($class ?? ''));
?>
<form action="<?= esc($logoutAction) ?>" method="POST" class="logout-form">
  <?= csrf_field() ?>
  <button type="submit" class="<?= esc($logoutClass) ?>" aria-label="<?= esc($logoutAriaLabel) ?>">
